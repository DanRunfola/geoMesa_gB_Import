...
export GEOMESA_HBASE_HOME="${GEOMESA_HBASE_HOME:-$(cd "`dirname "$0"`"/..; pwd)}"
export GEOMESA_CONF_DIR="${GEOMESA_CONF_DIR:-$GEOMESA_HBASE_HOME/conf}"
...
if [[ "$1" == "--help" || "$1" == "-h" || $# -eq 0 ]]; then
  ...
  echo ""
  echo "${usage}"
  echo "The \$GEOMESA_HBASE_HOME and \$GEOSERVER_HOME variables will be used if they are set and no path"
  ...
  if [[ -d "${GEOMESA_HBASE_HOME}" && -z "${gs_plugin_dir}" ]]; then
    gs_plugin_dir="${GEOMESA_HBASE_HOME}/dist/gs-plugins"
  fi
