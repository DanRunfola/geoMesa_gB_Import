...
export GEOMESA_CHECK_DEPENDENCIES="false"
...
# lib directory, by default $GEOMESA_HBASE_HOME/lib
export GEOMESA_LIB="${GEOMESA_LIB:-$GEOMESA_HBASE_HOME/lib}"

# logs directory, by default $GEOMESA_HBASE_HOME/logs
# this directory needs to be writable - in multi-tenant environments, each user should specify this differently
export GEOMESA_LOG_DIR="${GEOMESA_LOG_DIR:-$GEOMESA_HBASE_HOME/logs}"
...
function get_options() {
  ...
  GEOMESA_OPTS="${GEOMESA_OPTS} -Dgeomesa.home=${GEOMESA_HBASE_HOME}"
...
function geomesa_scala_console() {
  ...
  if [[ -d "${GEOMESA_HBASE_HOME}/dist/scala-2.11.7/" ]]; then
    scalaCMD="${GEOMESA_HBASE_HOME}/dist/scala-2.11.7/bin/scala"
  elif [[ $(which scala > /dev/null) -eq 0 && -n "$(scala -version 2>&1 | grep 2.11)" ]]; then
    scalaCMD="scala"
  else
    read -r -p "Download scala 2.11 (y/n)? " confirm
    confirm=${confirm,,} # lower-casing
    if [[ $confirm =~ ^(yes|y) || $confirm == "" ]]; then
      sourceURL=("https://downloads.lightbend.com/scala/2.11.7/scala-2.11.7.tgz")
      outputDir="${GEOMESA_HBASE_HOME}/dist/"
      outputFile="${outputDir}/scala-2.11.7.tgz"
      download_urls ${outputDir} sourceURL[@]
      if [[ $? -ne 0 ]]; then
        exit 1
      fi
      tar xf $outputFile -C "${GEOMESA_HBASE_HOME}/dist/"
      scalaCMD="${GEOMESA_HBASE_HOME}/dist/scala-2.11.7/bin/scala"
    else
      echo >&2 "Please install Scala version 2.11 and re-run this script"
      exit 1
    fi
  fi
...
function geomesa_configure() {
  ...
  echo >&2 "  GEOMESA_HBASE_HOME=${GEOMESA_HBASE_HOME}"
  ...
  if [[ $confirm =~ ^(yes|y) || $confirm == "" ]]; then
    ...
    read -r -p "Enter new value for GEOMESA_HBASE_HOME: " newHome
  ...
  if [[ $confirm =~ ^(yes|y) || $confirm == "" ]]; then
    echo "export GEOMESA_HBASE_HOME=\"$GEOMESA_HBASE_HOME\"" >> ~/.bashrc
    ...
    echo >&2 "export GEOMESA_HBASE_HOME=\"$GEOMESA_HBASE_HOME\""
    echo >&2 "export GEOMESA_LIB=\"${GEOMESA_LIB}\""
    echo >&2 "export GEOMESA_LOG_DIR=\"${GEOMESA_LOG_DIR}\""
    echo >&2 "export PATH=\${GEOMESA_HBASE_HOME}/bin:\$PATH"
  fi
